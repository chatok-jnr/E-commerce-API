...
- Role-based access control (`ADMIN`, `CUSTOMER`) via a many-to-many `UserRole` join table
- Profile management, including profile image upload
- Category CRUD (admin-only writes, public reads)
...
- `refresh_token` — longer-lived (7 days), used to obtain a new access token via `POST /auth/refresh`, rotated on each use

When testing in Postman, make sure cookies are enabled for your workspace — they'll persist automatically across requests once you log in.
